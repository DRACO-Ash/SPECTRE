...
# Rules chosen to approximate the SonarQube Python profile. Deliberately
...
#   S101    assert is how a test suite is written
#   PTH123  open() versus Path.open() is a style preference, not a Sonar rule
#   PLR2004 a status code in a test assertion IS the assertion
#   TRY003, EM101, EM102  long, specific exception messages are deliberate here:
#           the errno and remedy in the message are the diagnostic feature
...
#   ARG     pytest injects fixtures that a test body may not reference, and
#           FastAPI route signatures carry parameters the handler may not use
RULES="F,E,W,B,SIM,C90,N,S,BLE,A,C4,DTZ,T20,PIE,RET,SLF,ERA,PL,TRY400,PERF,LOG,G,RUF100"
IGNORED="E501,B008,S101,PTH123,PLR2004,PLR0913,TRY003,EM101,EM102,PLC0415"

pass() { printf '  PASS  %s\n' "$1"; }
fail() { printf '  FAIL  %s\n' "$1"; exit 1; }

changed_py() {
    git diff --name-only "$BASE...HEAD" -- '*.py' 2>/dev/null | while read -r f; do
        [ -f "$f" ] && printf '%s\n' "$f"
    done
}
...
FILES=$(changed_py | tr '\n' ' ')
if [ -z "$FILES" ]; then
    printf '\nNo changed Python files against %s. Nothing to check.\n\n' "$BASE"
    exit 0
fi

printf '\n== Static analysis on changed LINES (bugs, vulnerabilities, smells) ==\n'
# Findings are filtered to lines this branch actually changed, matching the
# gate's new-code scope. A pre-existing smell in a touched file is not a new
# violation and must not send anyone refactoring untested legacy code.
# shellcheck disable=SC2086
ruff check --no-cache --isolated --line-length 120 --target-version py312 \
    --select "$RULES" --ignore "$IGNORED" --output-format=json --exit-zero $FILES \
    | python3 scripts/sonar_scope.py "$BASE" && SCOPED_OK=0 || SCOPED_OK=1
