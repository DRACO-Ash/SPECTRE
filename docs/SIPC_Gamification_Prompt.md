# Claude Prompt: Gamified Training Environment for the Space Intercept Planning Console

> **Purpose:** Paste this prompt into Claude when you're ready to have it build the gamified training environment.  
> **Pre-requisite:** Claude should already have the SIPC codebase in context (uploaded or in-session).  
...
- They work under time pressure and need to build muscle memory for the SIPC workflow.
...
- **Step-by-step guided walkthroughs** that overlay the actual SIPC interface (not a separate documentation page). The operator learns by doing, with contextual guidance appearing as they interact with real controls.
...
- **Tutorials should cover these operator workflows** (adapt to match your actual SIPC features):
...
- How the new code integrates with the existing SIPC codebase.
...
1. **Architecture overview** — how the sandbox, tutorial engine, and gamification engine fit together and integrate with the existing SIPC codebase. Include a clear data isolation diagram.
