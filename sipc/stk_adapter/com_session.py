...
def _stk_dispatch() -> Any:
    """Return an ``STK13.Application`` COM object (late binding).

    Uses plain ``win32com.client.Dispatch`` so that all return values from COM
    method calls are wrapped as generic ``IDispatch`` objects rather than typed
    gen_py stubs.  The gen_py stubs generated by ``EnsureDispatch`` are
    incomplete for STK 13 (several interface modules are missing), causing
    ``ModuleNotFoundError`` when methods like ``GetObjectFromPath`` are called.

    Where the runtime COM object returns a sub-interface as ``IUnknown``
    (showing as ``<unknown>`` in Python), the caller must explicitly
    ``QueryInterface(pythoncom.IID_IDispatch)`` and re-wrap with
    ``win32com.client.Dispatch()`` to reach the runtime dispatch table.
    """
